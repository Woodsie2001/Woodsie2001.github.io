...
var rainParticlesArray = [];

var spd;
var minSpeed = 10;
var maxSpeed = 20;

var rawAmount = document.querySelector('input').value
var rainAmount = parseInt(rawAmount, 10)

for(var i = 0; i < rainAmount; i++){
  var xPos = getRandomInt(0 + 5, innerWidth - 5);
  var yPos = getRandomInt(0, innerHeight);
  var spd = getRandomInt(minSpeed, maxSpeed);
  rainParticlesArray.push(new RainParticle(xPos, yPos, spd))  
}
...
function RainParticle(x, y, speed){
  this.x = x;
  this.y = y;
  this.width = 3;
  this.height = getRandomInt(20, 40);
  this.speed = speed;
...
function Drops(x,y,radius){
  ...
  this.y = y
  this.radius = radius
  this.xSpeed = getRandomInt(-4,4);
  this.ySpeed = getRandomInt(1,4);
...
function splat(x,y){
  for(var i = 0; i < getRandomInt(3,10); i++){
    testone.push(new Drops(x, y, getRandomInt(3,5)))
  }
}
...
function Draw(){
  ...
  rawAmount = document.querySelector('input').value
  rainAmount = parseInt(rawAmount, 10)
  
  if(rainAmount < 0){
    document.querySelector('input').value = 0
  }
  if(rainParticlesArray.length < rainAmount){
    var difference = rainAmount - rainParticlesArray.length
  ...
  }else if(rainParticlesArray.length > rainAmount){
    var dif = rainParticlesArray.length - rainAmount
    for(var i = 0; i < dif; i++){
      rainParticlesArray.splice(rainParticlesArray.length - i, 1)
    }
  }
  
  for(var i = 0; i < rainParticlesArray.length; i++){
    rainParticlesArray[i].update();
    if(rainParticlesArray[i].y > canvas.height){
      splat(rainParticlesArray[i].x,rainParticlesArray[i].y);
      rainParticlesArray.splice(i,1);
      
      var xPos = getRandomInt(0 + 5, innerWidth - 5);
      var yPos = 0 - 30;
      var spd = getRandomInt(minSpeed, maxSpeed);
      rainParticlesArray.splice(i, 0, new RainParticle(xPos, yPos, spd));
    }
  }
